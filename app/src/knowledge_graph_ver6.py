...
# Create a graph with filtered words above a certain edge weight threshold
G = nx.Graph()
edge_weight_threshold = 50  # Define the threshold for edge weights
for (word1, word2), count in co_occurrence.items():
    if count >= edge_weight_threshold:  # Only add edges with weight above the threshold
